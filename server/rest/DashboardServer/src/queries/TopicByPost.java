...
@Path("/Contenuto/idpost={IDPost}")
public class TopicByPost {
	...
	@DELETE
	public void deleteContenuto(@PathParam("IDPost") int idp) {
		try {
			Connection conn = DbManager.getConnection();
			String query = "DELETE from Contenuto where IDPost = " + idp + ";";
	...
	public void updateContenuto(@PathParam("IDpost") int idp, String payload) {
		try {
	...
	private Contenuto getContenuto(int idp) throws Exception {
		Contenuto contenuto = null;
		Connection conn = DbManager.getConnection();
		Statement stmt = conn.createStatement();
		String query = "SELECT * from Contenuto where IDpost = " + idp + ";";
		ResultSet rset = stmt.executeQuery(query);
		if (rset.next()) {
