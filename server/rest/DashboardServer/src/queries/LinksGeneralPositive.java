//SELECT p.Link AS label FROM Post AS p INNER JOIN Contenuto AS c INNER JOIN Topics AS t WHERE p.ID = c.IDPost AND c.IDTopic=t.ID AND t.Topic = '" + topic + "';
...
@Path("/links/general/positive/idfonte={IDFonte}/limit={inf}-{sup}")
public class LinksGeneralPositive {
	...
	private List<String> getLinks(int id,int inf, int sup) throws Exception {
		...
		ResultSet rset = stmt.executeQuery("SELECT p.Link AS label FROM Sorgenti AS s INNER JOIN  Post AS p INNER JOIN Contenuto AS c INNER JOIN Topics AS t WHERE p.ID = c.IDPost AND c.IDTopic=t.ID AND p.ID_Fonte = s.ID AND p.ID_Fonte ="+id+" AND p.Polarity ='+1' LIMIT "+inf+" , "+sup+";");
		while (rset.next()) {
