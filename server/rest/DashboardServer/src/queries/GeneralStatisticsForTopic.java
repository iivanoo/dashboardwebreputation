//restituisce il numero dei post tra due date per la fonte cliccata tenendo conto del topic
package queries;
...
public class GeneralStatisticsForTopic{
	...
	public String getGeneralStatistics(@PathParam("NomeFonte") String nome, @PathParam("topic") String topic, @PathParam("Data1") String d1, @PathParam("Data2") String d2, @PathParam("Data3") String d3, @PathParam("Data4") String d4, @PathParam("Data5") String d5, @PathParam("Data6") String d6, @PathParam("Data7") String d7) {
		String result = "";
		try {
			
			result = this.getSorgenti(nome,topic,d1,d2,d3,d4,d5,d6,d7);
	...
	private String getSorgenti(String nome,String topic,  String d1, String d2, String d3, String d4, String d5, String d6, String d7) throws Exception {
		int count;
		Connection conn;
		Statement stmt;
		ResultSet rset;
...
conn = DbManager.getConnection();
stmt = conn.createStatement();
rset = stmt.executeQuery("SELECT p.ID FROM Sorgenti AS s INNER JOIN Post AS p INNER JOIN Contenuto AS c INNER JOIN Topics AS t WHERE s.ID = p.ID_Fonte AND p.ID = c.IDPost AND c.IDTopic = t.ID AND t.Topic= '"+topic+"' AND p.Data BETWEEN '"+d6+"' AND '"+d7+"'  AND s.Nome = '"+nome+"'");
count = 0;
...
conn_neg = DbManager.getConnection();
stmt_neg = conn_neg.createStatement();
rset_neg = stmt_neg.executeQuery("SELECT p.Polarity FROM Sorgenti AS s INNER JOIN Post AS p INNER JOIN Contenuto AS c INNER JOIN Topics AS t WHERE s.ID = p.ID_Fonte AND p.ID = c.IDPost AND c.IDTopic = t.ID AND t.Topic = '"+topic+"' AND p.Polarity = '-1' AND s.Nome = '"+nome+"' AND p.Data BETWEEN '"+d6+"' AND '"+d7+"';");
count_neg = 0;
while (rset_neg.next()) {
	count_neg++;
	neg6.add(count_neg);
...
conn_pos = DbManager.getConnection();
stmt_pos = conn_pos.createStatement();
rset_pos = stmt_pos.executeQuery("SELECT p.Polarity FROM Sorgenti AS s INNER JOIN Post AS p INNER JOIN Contenuto AS c INNER JOIN Topics AS t WHERE s.ID = p.ID_Fonte AND p.ID = c.IDPost AND c.IDTopic = t.ID AND t.Topic = '"+topic+"' AND p.Polarity = '+1' AND s.Nome = '"+nome+"' AND p.Data BETWEEN '"+d6+"' AND '"+d7+"';");
count_pos = 0;
while (rset_pos.next()) {
	count_pos++;
	pos6.add(count_pos);
