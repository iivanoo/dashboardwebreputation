...
@Path("/links/topic/positive/idfonte={IDFonte}/topic={topic}/limit={inf}-{sup}")
public class LinksForTopicPositive {
	...
	private List<String> getLinks(int id, String topic, int inf, int sup) throws Exception {
		...
		ResultSet rset = stmt.executeQuery("SELECT p.Link AS label FROM Sorgenti AS s INNER JOIN  Post AS p INNER JOIN Contenuto AS c INNER JOIN Topics AS t WHERE p.ID = c.IDPost AND c.IDTopic=t.ID AND p.ID_Fonte = s.ID AND p.ID_Fonte ="+id+" AND t.Topic = '" + topic + "' AND p.Polarity = '+1' LIMIT "+inf+" , "+sup+";");
		while (rset.next()) {
