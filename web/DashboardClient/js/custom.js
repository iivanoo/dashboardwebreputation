...
/*	
	if($("#sincos").length)
	{
		//array con il numero dei post settimanali
		...
		//inserimento dati
			...
			//myDate.setDate(myDate.getDate()+7);
				...
				dt.push([data_post[i], numero_post[i]]);
				}
...
/*	if($("#plotmultiplo").length)
	{
		...
		$("#plotmultiplo").bind("plotclick", function (event, pos, item) {
			if (item) {
				$("#clickdata").text("You clicked point " + item.dataIndex + " in " + item.series.label + ".");
				plot.highlight(item.series, item.datapoint);
			}
...
/*	if($("#flotchart").length)
	{
		...
		//inserimento dati
			...
			//myDate.setDate(myDate.getDate()+7);
				...
				d1.push([data_post[i], pol_post[i]]);
				}
